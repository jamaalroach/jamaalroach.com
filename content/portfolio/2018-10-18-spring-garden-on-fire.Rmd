...
temps50<-temps%>%filter(Year>1965)
ggplot(temps50, aes(x=Month, y=Temp, group=Year))+geom_line()+theme_void()
```
```{r}
yavg100<-yavg%>%filter(Year>1915)
barplot(yavg100$AVgTemp, yavg100$Year)
...
```{r}
barplot(dec$maxtemp , dec$decade, space =.05, )
```
```{r}
barplot(yavg$AVgTemp , yavg$Year, space =5 )
```
...
colfunc<-colorRampPalette(c("red","royalblue"))
 plot(davg$AVgTemp, rev(davg$decade), pch=15, col=colfunc(davg$AVgTemp) )
